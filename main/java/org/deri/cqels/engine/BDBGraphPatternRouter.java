...
//		System.out.println(binding);
		...
		for(int i = 0; i < vars.size(); i++) { 
			...
			j++;
			System.out.println("for "+i+"= "+"count="+j+"encoded value ="+context.engine().encode(binding.get(vars.get(i))));
		}
	...
	//	System.out.println("weight" +weight);
		if(weight > 0) {
			if(weight < indexes.get(idx).size()) {
				...
				TupleOutput out = new TupleOutput();
				for(int i = 0; i < idxMask.size(); i++) {
					// print the mapping --- yashpal
//					System.out.println("search buff print1"+vars.get(idxMask.get(i)));
//					System.out.println("search buff print2"+mapping.get(vars.get(idxMask.get(i))));
					out.writeLong(mapping.get(vars.get(idxMask.get(i))));
